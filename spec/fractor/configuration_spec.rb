...
RSpec.describe Fractor::Configuration do
  ...
  describe ".configure" do
    ...
    it "allows setting configuration values" do
      ...
      expect(described_class.instance.default_worker_timeout).to eq(60)
    end

    it "returns the configuration instance" do
      result = described_class.configure { |c| c.debug = true }
  ...
  describe ".configure_from_file" do
    ...
    it "loads configuration from YAML file" do
      described_class.configure_from_file(temp_file.path)
    ...
    it "raises ArgumentError for non-existent file" do
      expect do
        described_class.configure_from_file("nonexistent.yml")
  ...
  describe "#validate!" do
    ...
    it "raises ConfigurationError for invalid timeout" do
      ...
      expect do
        config.validate!
      end.to raise_error(Fractor::ConfigurationError, /must be positive/)
    end
    ...
    it "raises ConfigurationError for negative max retries" do
      ...
      expect do
        config.validate!
      end.to raise_error(Fractor::ConfigurationError, /must be non-negative/)
    end
  ...
  describe "Fractor.config" do
    it "provides convenient access to configuration instance" do
      expect(Fractor.config).to be_a(described_class)
    end
  end
