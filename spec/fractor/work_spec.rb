...
RSpec.describe Fractor::Work do
  describe "#initialize" do
    it "stores the input data" do
      ...
      work = described_class.new(input_data)
    ...
    it "works with various input types" do
      # Test with a number
      work1 = described_class.new(42)
      expect(work1.input).to eq(42)

      # Test with an array
      work2 = described_class.new([1, 2, 3])
      expect(work2.input).to eq([1, 2, 3])

      # Test with a hash
      work3 = described_class.new({ key: "value" })
      expect(work3.input).to eq({ key: "value" })

      # Test with nil
      work4 = described_class.new(nil)
  ...
  describe "subclassing" do
    it "allows customization through subclassing" do
      ...
      # Verify inheritance
      expect(custom_work).to be_a(described_class)
